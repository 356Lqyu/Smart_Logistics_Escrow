...
async function loadShipperDashboard() {
  ...
  const mine = (data || []).filter(
    (a) => a.shipper_address && a.shipper_address.toLowerCase() === wallet,
  );
  ...
  const completed = mine.filter((a) => normalize(a.status) === "completed");
    ...
    body.innerHTML = `
            <tr>
                <td colspan="7" class="dash-empty-row" >
                    No agreements yet.
                    <a href="createAgreement.html" style="color: #6ab2ff;">Create your first agreement</a>
                </td>
            </tr>
        `;
    return;
...
function renderShipperChart(agreements) {
  ...
  agreements.forEach((job) => {
    ...
    const createdAt =
      ts > 0 ? new Date(ts) : new Date(job.created_at || Date.now());
