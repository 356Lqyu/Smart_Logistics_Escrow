document.addEventListener("DOMContentLoaded", async () => {
    try {
        ...
        document.getElementById("tx-event-type").innerText = transaction.event_type || "Transaction";
        ...
        const rawTxHash = transaction.transaction_hash;
        ...
        document.getElementById("tx-timestamp").innerText = formatDateTime(transaction.created_at);

        const detailsObj = transaction.details || {};
        document.getElementById("tx-description").innerText = detailsObj.description || detailsObj.status || "No description provided.";
...
function getTransactionStatus(eventType) {
    ...
    if (type.includes("extension") || type.includes("extended")) return "Extended";
    ...
    if (type.includes("verified") || type.includes("payout") || type.includes("release")) return "Verified";
    return "Recorded";
}
...
function getStatusClass(status) {
    switch (status) {
        case "Funded":
        case "Extended": return "status-funded";
        case "Submitted": return "status-submitted";
        case "Verified":
        case "Released": return "status-verified";
